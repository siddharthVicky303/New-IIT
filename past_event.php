        ...
        if (!empty($displayData)) {
            foreach ($displayData as $row) {
                ...
                foreach ($row as $index => $cell) {
                    $cell = trim($cell); // Remove extra spaces
            
                    // If the cell contains a valid URL, make it clickable
                    if (filter_var($cell, FILTER_VALIDATE_URL)) {  
                        echo "<td><a href='" . htmlspecialchars($cell) . "' target='_blank'>" . htmlspecialchars($cell) . "</a></td>";
                    } 
                    // If the cell already contains an <a> tag, allow rendering
                    elseif (strpos($cell, "<a") !== false) {
                        echo "<td>" . $cell . "</td>"; // Allow HTML rendering
                    } 
                    else {
